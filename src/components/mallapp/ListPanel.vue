<template>
  <div ref="listPanel">
    <div ref="listBox" class="app-list-body" :class="{'app-list-body-horizontal': !vertical,'app-list-body-disabled': clacable || refreshing}"
         @scroll="scroll">
      <slot></slot>
    ...
    methods: {
      ...
      touchStart: function (e) {
        if (!this.refreshing) {
          ...
          this.startDistance = this.vertical ? e.targetTouches[0].pageY : e.targetTouches[0].pageX
          this.startScrollPosition = this.vertical ? this.$refs.listBox.scrollTop : this.$refs.listBox.scrollLeft
          if (this.refreshable && e.targetTouches.length === 1 && ((this.vertical && this.startScrollPosition === 0) || (!this.vertical && this.startScrollPosition === 0))) {
      ...
      touchMove: function (e) {
        if (!this.refreshing) {
          let currentPosition = this.vertical ? e.targetTouches[0].pageY : e.targetTouches[0].pageX
          let moveDistance = currentPosition - this.startDistance
          if (this.vertical) {
            this.$refs.listBox.scrollTop = this.startScrollPosition - moveDistance
          } else {
            this.$refs.listBox.scrollLeft = this.startScrollPosition - moveDistance
          }
          if (this.isRefreshable) {
            if (e.targetTouches.length === 1) {
              if (this.checkFigner === false && moveDistance > 0) {
                this.clacable = true
              }
              this.checkFigner = true
              if (this.refreshable && this.clacable) {
                let distance = currentPosition - this.startDistance
