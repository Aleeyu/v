<template>
  <Row class="container">
    <Row v-show="current === 'CHART'">
      <Query :show-dealer="false" 
        :show-area-type="false" 
        :show-dealer-level="true"
        :show-order="true"
  ...
  methods: {
    ...
    init () {
      ...
      this.chart.instance.on('click', params => {
        let data = this.chart.data.find(d => d.dealerName === params.name)
        if (data) {
          this.info.dealerId = data.dealerId
          this.getInfo()
        } else {
          this.$Message.error('获取经销商相关的信息失败！')
        }
      })
    },
    render () {
      let data = {
        _loading: true,
        fieldName: this.condition.order.fieldName === 'default' ? 'dealerName' : this.condition.order.fieldName,
      ...
      if (this.condition.dealerLevel) {
        data.dealerLevel = this.condition.dealerLevel
      }

      if (this.error) {
        this.errorLoading = true
      }
      this.$http.post('/order/dealer/analysis', data).then(res => {
        if (res.length === 0) {
          this.error = {
            type: 2,
            title: '没有对应条件的数据',
    ...
    getInfo (page) {
      ...
      if (this.condition.dealerLevel) {
        data.queryFilter.push({
          property: 'dealerLevel',
          op: '=',
          value: this.condition.dealerLevel
        })
      }
